    ...
    <PanelCard className="p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={onSave}
            disabled={saving}
            className="rounded-xl border border-blue-400/30 bg-blue-500/15 px-3 py-2 text-sm font-medium text-blue-100 transition hover:bg-blue-500/20 disabled:opacity-60"
          >
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-2">
        {stages.map((stage, index) => (
          <div key={`${stage.id}_${index}`} className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
            <div className="grid gap-3 md:grid-cols-[1fr_1fr_88px]">
              <label className="space-y-1 text-xs text-white/52">
                <span>Nome</span>
                <input
                  value={stage.label}
              ...
              <label className="space-y-1 text-xs text-white/52">
                ...
                <input
                  ...
                  className="h-[42px] w-full rounded-xl border border-white/10 bg-black/30 p-1"
                />
              </label>
            </div>
          </div>
      ...
      <EmptyState
        ...
        action={
          <button
            ...
            className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-white/86 transition hover:bg-white/[0.06]"
          >
            Recarregar pipeline
          </button>
        }
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canManage ? (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-white/82 transition hover:bg-white/[0.06]"
              >
      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <PanelCard className="p-4">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <div className="inline-flex rounded-xl border border-blue-400/20 bg-blue-500/10 p-2 text-blue-100">
                <Sparkles className="h-4 w-4" />
              </div>
              <p className="mt-3 text-sm text-white/58">Maior concentracao</p>
              <p className="mt-1 text-xl font-semibold text-white">{topStage?.stage.label || "Sem dados"}</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <p className="mt-3 text-sm text-white/58">Gargalo atual</p>
              <p className="mt-1 text-xl font-semibold text-white">{stalledStage?.stage.label || "Sem gargalo"}</p>
              <p className="mt-2 text-sm text-white/52">
        ...
        <PanelCard className="p-4">
          ...
          <div className="mt-4 space-y-2">
            {stages.map((stage) => (
              <div key={stage.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3">
      ...
      <PanelCard className="p-4">
        ...
        <div className="mt-4 grid gap-3 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <p className="mt-2 text-sm text-white/60">{businessProfile.description}</p>
            <p className="mt-3 text-sm text-white/72">
              Movimento esperado: <span className="text-white">{businessProfile.commercialMotion}</span>
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <p className="text-sm font-semibold text-white">Metricas naturais deste modo</p>
        ...
        <PanelCard className="p-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              />
              <p className="mt-3 text-lg font-semibold text-white">{selectedLeadContext.lead.nome}</p>
                  ...
                  className={`flex w-[320px] shrink-0 flex-col rounded-2xl border p-3 transition ${
                    isDropTarget
                      ...
                      : isStageFocus || isLeadFocus
                        ? "border-blue-300/25 bg-blue-500/[0.04]"
                ...
                >
                  <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm font-semibold text-white">{column.stage.label}</p>
                        </div>
                        <p className="mt-1 text-xs leading-5 text-white/52">{column.stage.description}</p>
                      </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
                        <p className="text-[11px] uppercase tracking-[0.12em] text-white/42">Valor</p>
                        <p className="mt-1 text-sm font-medium text-white">{brl(column.totalValue)}</p>
                      </div>
                      <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
                        <p className="text-[11px] uppercase tracking-[0.12em] text-white/42">Tempo medio</p>
                        <p className="mt-1 text-sm font-medium text-white">{column.avgAgeDays}d</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex flex-1 flex-col gap-3">
                    {column.items.length === 0 ? (
                      <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-4 text-sm text-white/44">
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-white/48">{lead.empresa || lead.source || "Origem nao informada"}</p>
                            </div>
                            {canOperate ? <GripVertical className="mt-0.5 h-4 w-4 text-white/28" /> : null}
                          </div>
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-white/60">
                            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-2.5 py-2">
                              <p className="text-[10px] uppercase tracking-[0.12em] text-white/40">Valor</p>
                              <p className="mt-1 text-sm font-medium text-white">{brl(Number(lead.potentialValue || 0))}</p>
                            </div>
                            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-2.5 py-2">
                              <p className="text-[10px] uppercase tracking-[0.12em] text-white/40">Tempo</p>
                              <p className="mt-1 text-sm font-medium text-white">{lead.ageDays || 0}d</p>
                            </div>
                          </div>
                          ...
                          {lead.tags?.length ? (
                            <div className="mt-3 flex flex-wrap gap-1.5">
                              {lead.tags.slice(0, 3).map((tag) => (
                                <span key={tag} className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-1 text-[11px] text-white/62">
                                  #{tag}
                                </span>
                              ))}
                            </div>
                          ...
                          <div className="mt-4 flex items-center justify-between gap-2">
                            <div className="flex gap-2">
                              <Link
                                ...
                                className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-white/82 transition hover:bg-white/[0.06]"
                              >
                                Abrir lead
                            ...
                            {movingLeadId === lead.id ? <Loader2 className="h-4 w-4 animate-spin text-blue-200" /> : null}
                          </div>
                        </article>
                      ))
                    )}
