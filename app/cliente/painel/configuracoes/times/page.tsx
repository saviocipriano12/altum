...
type Member = { id?: string; userId?: string; name?: string; email?: string; role?: string; team?: string; availability?: string; status?: string; allowedChannels?: string[]; maxOpenChats?: number | null; accessProfile?: string; capabilities?: string[] };
...
type Channel = { id?: string; type?: string; displayName?: string; phoneNumber?: string; channelScope?: string; ownerUserId?: string; ownerUserName?: string; connectionStatus?: string; provider?: string; isOfficial?: boolean };
type ChannelsPayload = { items?: Channel[]; error?: string };
type Performance = { ownerId?: string; ownerName?: string; activeChats?: number; pendingChats?: number; overdueChats?: number; handledChats?: number; wonLeads?: number; totalLeads?: number; avgFirstResponseMinutes?: number };
...
export default function ClienteTimesPage() {
  ...
  const [personDraft, setPersonDraft] = useState<DraftPerson & { team: string }>({ ...blankPerson(), team: "" });

  async function loadData() {
    if (!tenant?.tenantId) return;
    try {
      setLoading(true); setError(null);
      const [settingsRes, usersRes, channelsRes, metricsRes] = await Promise.all([authedFetch(`/api/tenant/${tenant.tenantId}/settings`), authedFetch(`/api/tenant/${tenant.tenantId}/users`), authedFetch(`/api/tenant/${tenant.tenantId}/channels`), authedFetch(`/api/tenant/${tenant.tenantId}/metrics-summary`)]);
      const settings = await settingsRes.json() as SettingsPayload;
      const users = await usersRes.json() as UsersPayload;
      const channelsPayload = await channelsRes.json().catch(() => ({})) as ChannelsPayload;
      const metricsPayload = await metricsRes.json().catch(() => ({})) as { operations?: { teamPerformance?: Performance[] } };
      ...
      setTeams(nextTeams); setMembers(nextMembers); setSavedMembers(nextMembers); setChannels(channelsPayload.items || []); setPerformance(metricsPayload.operations?.teamPerformance || []);
  ...
  const teamChannels = useMemo(() => selected ? channels.filter((channel) => selected.channels.includes(String(channel.type || "").toLowerCase())) : [], [channels, selected]);
  const availablePeople = useMemo(() => {
    const query = existingSearch.trim().toLowerCase();
    return members.filter((member) => member.role !== "client_owner" && member.userId && member.team !== selected?.id && (!query || `${member.name || ""} ${member.email || ""}`.toLowerCase().includes(query)));
  }, [existingSearch, members, selected]);
  const selectedPerformance = useMemo(() => performance.filter((item) => selectedMembers.some((member) => member.userId === item.ownerId)), [performance, selectedMembers]);
  const stats = useMemo(() => ({
  ...
  async function saveMember(member: Member) {
    ...
    const keys = (["team", "availability", "allowedChannels", "maxOpenChats", "role", "accessProfile", "capabilities"] as const).filter((key) => JSON.stringify(member[key]) !== JSON.stringify(before?.[key]));
  ...
  async function assignExistingPeople() {
    ...
    try {
      ...
      const selectedPeople = members.filter((member) => member.userId && existingSelected.includes(member.userId));
      await Promise.all(selectedPeople.map((member) => saveMember({ ...member, team: selected.id })));
      setMembers((current) => current.map((member) => existingSelected.includes(String(member.userId)) ? { ...member, team: selected.id } : member));
      setExistingSelected([]); setExistingPickerOpen(false); setNotice(`${selectedPeople.length} pessoa(s) adicionada(s) ao time.`);
    } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível vincular as pessoas."); }
    finally { setSaving(false); }
  }
  ...
  async function invitePerson(event: FormEvent) {
    ...
    if (!personDraft.email.trim()) { setError("Informe o e-mail da pessoa."); return; }
    try {
      setPersonSaving(true); setError(null); setNotice(null);
      const profile = getClientAccessProfile(personDraft.accessProfile);
      const response = await authedFetch(`/api/tenant/${tenant.tenantId}/users`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: personDraft.name.trim(), email: personDraft.email.trim(), team: personDraft.team, role: profile.role, accessProfile: profile.id, availability: "online", allowedChannels: ["whatsapp"], maxOpenChats: Number(personDraft.maxOpenChats || 0), capabilities: profile.capabilities }) });
      const body = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) throw new Error(body.error || "Não foi possível adicionar a pessoa.");
      setPersonComposerOpen(false); setPersonDraft({ ...blankPerson(), team: selectedId || teams[0]?.id || "" });
      await loadData(); setNotice("Pessoa adicionada e convite preparado.");
    } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível adicionar a pessoa."); }
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-indigo-600">Central de operação</p><h2 className="mt-1 text-xl font-bold text-[var(--cliente-card-text)]">Times e pessoas no mesmo lugar</h2></div>{canManage && canManageUsers && <div className="flex flex-wrap gap-2"><button type="button" onClick={() => { setPersonDraft({ ...blankPerson(), team: selectedId || teams[0]?.id || "" }); setPersonComposerOpen(true); }} className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-2.5 text-sm font-bold text-indigo-700 hover:bg-indigo-50"><UserPlus className="h-4 w-4" />Adicionar pessoa</button><button type="button" onClick={() => setComposerOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-[var(--cliente-primary)] px-4 py-2.5 text-sm font-bold text-white hover:opacity-90"><Plus className="h-4 w-4" />Adicionar time</button></div>}</div>
    <section className="grid gap-4 xl:grid-cols-[minmax(300px,.75fr)_minmax(0,1.25fr)]">
      ...
      <PanelCard className="p-5">{selected ? <form onSubmit={saveTeams} className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-indigo-600">Time selecionado</p><h3 className="mt-1 text-xl font-bold text-[var(--cliente-card-text)]">{selected.name}</h3><p className="mt-1 text-sm text-[var(--cliente-card-text-muted)]">Ajuste identidade, canais e pessoas sem sair desta tela.</p></div>{canManage && <button type="button" onClick={removeSelectedTeam} className="inline-flex items-center gap-2 rounded-xl border border-red-200 px-3 py-2 text-xs font-bold text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" />Remover</button>}</div><div className="grid gap-3 sm:grid-cols-2"><Field label="Nome do time" value={selected.name} disabled={!canManage} onChange={(value) => changeTeam({ name: value })} /><Field label="Objetivo" value={selected.description || ""} disabled={!canManage} onChange={(value) => changeTeam({ description: value })} /></div><label className="block space-y-1.5"><span className="text-xs font-bold text-[var(--cliente-card-text-soft)]">Time padrão para novas conversas</span><select value={defaultTeam} disabled={!canManage} onChange={(event) => setDefaultTeam(event.target.value)} className="w-full rounded-xl border border-[var(--cliente-border)] bg-white px-3 py-2.5 text-sm"><option value="">Não definir</option>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label><div><p className="text-xs font-bold text-[var(--cliente-card-text-soft)]">Canais atendidos</p><div className="mt-2 flex flex-wrap gap-2">{CHANNELS.map((channel) => { const active = selected.channels.includes(channel.id); return <button type="button" key={channel.id} disabled={!canManage} onClick={() => changeTeam({ channels: toggle(selected.channels, channel.id) })} className={`rounded-full border px-3 py-1.5 text-xs font-bold ${active ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-[var(--cliente-border)] bg-white text-[var(--cliente-card-text-soft)]"}`}>{active && <Check className="mr-1 inline h-3 w-3" />}{channel.label}</button>})}</div></div><div className="border-t border-[var(--cliente-border)] pt-5"><div className="flex items-center justify-between gap-3"><div><p className="font-bold text-[var(--cliente-card-text)]">Pessoas deste time</p><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">Disponibilidade e capacidade de atendimento.</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{selectedMembers.length}</span>{canManageUsers && <button type="button" onClick={() => setExistingPickerOpen(true)} className="inline-flex items-center gap-1.5 rounded-xl border border-indigo-200 bg-white px-2.5 py-1.5 text-xs font-bold text-indigo-700"><UserPlus className="h-3.5 w-3.5" />Adicionar cadastrados</button>}</div></div><div className="mt-3 space-y-3">{selectedMembers.map((member) => { const index = members.findIndex((item) => item.userId === member.userId); const locked = !canManageUsers || member.role === "client_owner"; return <div key={member.userId || member.email} className="rounded-2xl border border-[var(--cliente-border)] bg-[var(--cliente-surface-muted)] p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-bold text-[var(--cliente-card-text)]">{member.name || "Sem nome"}</p><p className="truncate text-xs text-[var(--cliente-card-text-soft)]">{member.email}</p></div><StateBadge label={member.role === "client_owner" ? "Dono" : inferClientAccessProfile(member).label} tone="info" /></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><select disabled={locked} value={inferClientAccessProfile(member).id} onChange={(event) => { const profile = getClientAccessProfile(event.target.value); changeMember(index, { accessProfile: profile.id, role: profile.role, capabilities: profile.capabilities }); }} className="rounded-xl border border-[var(--cliente-border)] bg-white px-2.5 py-2 text-xs"><option value="seller">Vendedor</option><option value="manager">Gestor</option><option value="support">Atendimento</option><option value="analyst">Analista</option></select><select disabled={locked} value={member.availability || "online"} onChange={(event) => changeMember(index, { availability: event.target.value })} className="rounded-xl border border-[var(--cliente-border)] bg-white px-2.5 py-2 text-xs"><option value="online">Disponível</option><option value="busy">Ocupado</option><option value="offline">Fora da escala</option></select><input disabled={locked} type="number" min={1} max={200} value={member.maxOpenChats ?? ""} placeholder="Capacidade" onChange={(event) => changeMember(index, { maxOpenChats: event.target.value ? Number(event.target.value) : null })} className="rounded-xl border border-[var(--cliente-border)] bg-white px-2.5 py-2 text-xs" /><button type="button" disabled={locked} onClick={() => void saveMember(member).then(() => setNotice("Pessoa atualizada.")).catch((e) => setError(e instanceof Error ? e.message : "Falha ao salvar pessoa."))} className="inline-flex items-center justify-center gap-1 rounded-xl border border-indigo-200 bg-white px-2.5 py-2 text-xs font-bold text-indigo-700"><Save className="h-3.5 w-3.5" />Salvar</button></div></div>})}{!selectedMembers.length && <p className="rounded-xl border border-dashed border-[var(--cliente-border)] p-4 text-sm text-[var(--cliente-card-text-soft)]">Nenhuma pessoa vinculada a este time ainda.</p>}</div></div><div className="grid gap-3 border-t border-[var(--cliente-border)] pt-5 sm:grid-cols-2"><div><p className="font-bold text-[var(--cliente-card-text)]">Canais ligados</p><div className="mt-2 space-y-2">{teamChannels.length ? teamChannels.map((channel) => <div key={channel.id || channel.displayName} className="rounded-xl border border-[var(--cliente-border)] bg-white p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-bold text-[var(--cliente-card-text)]">{channel.displayName || channel.phoneNumber || "Canal"}</p><StateBadge label={channel.connectionStatus === "connected" || channel.connectionStatus === "ready" ? "Conectado" : "Revisar"} tone={channel.connectionStatus === "connected" || channel.connectionStatus === "ready" ? "success" : "warning"} /></div><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">{channel.phoneNumber || "Número não informado"} · {channel.channelScope === "personal" ? channel.ownerUserName || "Uso pessoal" : "Canal da empresa"}</p></div>) : <p className="rounded-xl border border-dashed border-[var(--cliente-border)] p-3 text-xs text-[var(--cliente-card-text-soft)]">Nenhum canal ligado a este time.</p>}</div></div><div><div className="flex items-center justify-between gap-2"><p className="font-bold text-[var(--cliente-card-text)]">Desempenho do time</p><Link href="/cliente/painel/relatorios?section=sellers" className="text-xs font-bold text-indigo-700">Ver ranking completo</Link></div><div className="mt-2 space-y-2">{selectedPerformance.length ? selectedPerformance.slice(0, 5).sort((a, b) => Number(b.wonLeads || 0) - Number(a.wonLeads || 0)).map((item, index) => <div key={item.ownerId || item.ownerName} className="flex items-center justify-between rounded-xl border border-[var(--cliente-border)] bg-white p-3"><div><p className="text-xs font-bold text-[var(--cliente-card-text)]">{index + 1}. {item.ownerName || "Vendedor"}</p><p className="mt-1 text-[11px] text-[var(--cliente-card-text-soft)]">{item.handledChats || 0} atendimentos · {item.avgFirstResponseMinutes || 0} min de resposta</p></div><span className="text-xs font-black text-emerald-700">{item.wonLeads || 0} vendas</span></div>) : <p className="rounded-xl border border-dashed border-[var(--cliente-border)] p-3 text-xs text-[var(--cliente-card-text-soft)]">O ranking aparecerá quando houver atividade atribuída.</p>}</div></div></div>{canManage && <div className="flex justify-end border-t border-[var(--cliente-border)] pt-4"><button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[var(--cliente-primary)] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><Save className="h-4 w-4" />{saving ? "Salvando" : "Salvar time"}</button></div>}</form> : <EmptyState onCreate={() => setComposerOpen(true)} canCreate={canManage && canManageUsers} />}</PanelCard>
    </section>
    {selected && <TeamInsights channels={teamChannels} performance={selectedPerformance} />}
    <PeopleDirectory members={members} teams={teams} search={directorySearch} setSearch={setDirectorySearch} canManage={canManageUsers} onChange={changeMember} onSave={(member) => void saveMember(member).then(() => setNotice("Pessoa atualizada.")).catch((e) => setError(e instanceof Error ? e.message : "Falha ao salvar pessoa."))} />
    ...
    {existingPickerOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 p-0 sm:items-center sm:p-6"><div role="dialog" aria-modal="true" className="max-h-[88vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-indigo-600">Pessoas cadastradas</p><h2 className="mt-1 text-xl font-bold text-slate-950">Adicionar ao time {selected?.name}</h2><p className="mt-1 text-sm text-slate-600">Selecione quem já possui acesso à empresa.</p></div><button type="button" onClick={() => setExistingPickerOpen(false)} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><input value={existingSearch} onChange={(event) => setExistingSearch(event.target.value)} placeholder="Buscar por nome ou e-mail" className="mt-5 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-indigo-300" /><div className="mt-3 space-y-2">{availablePeople.map((person) => <label key={person.userId} className="flex cursor-pointer items-center gap-3 rounded-2xl border border-slate-200 p-3 hover:bg-indigo-50"><input type="checkbox" checked={existingSelected.includes(String(person.userId))} onChange={() => setExistingSelected((current) => current.includes(String(person.userId)) ? current.filter((id) => id !== person.userId) : [...current, String(person.userId)])} className="h-4 w-4 accent-indigo-600" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-slate-900">{person.name || "Sem nome"}</span><span className="block truncate text-xs text-slate-500">{person.email} · {person.team ? `Time atual: ${person.team}` : "Sem time"}</span></span><StateBadge label={inferClientAccessProfile(person).label} tone="info" /></label>)}{!availablePeople.length && <p className="rounded-xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">Nenhuma pessoa disponível para adicionar.</p>}</div><div className="mt-5 flex justify-end gap-2 border-t border-slate-200 pt-4"><button type="button" onClick={() => setExistingPickerOpen(false)} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-600">Cancelar</button><button type="button" disabled={!existingSelected.length || saving} onClick={() => void assignExistingPeople()} className="rounded-xl bg-[var(--cliente-primary)] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">Adicionar selecionadas</button></div></div></div>}
    {personComposerOpen && <PersonComposer draft={personDraft} setDraft={setPersonDraft} teams={teams} saving={personSaving} onClose={() => setPersonComposerOpen(false)} onSubmit={invitePerson} />}
...
function TeamInsights({ channels, performance }: { channels: Channel[]; performance: Performance[] }) { return <section className="grid gap-4 md:grid-cols-2"><PanelCard className="p-5"><div className="flex items-center justify-between"><div><p className="font-bold text-[var(--cliente-card-text)]">Canais ligados ao time</p><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">Veja quais números a equipe pode usar.</p></div><Link href="/cliente/painel/configuracoes/canais" className="text-xs font-bold text-indigo-700">Gerenciar canais</Link></div><div className="mt-4 space-y-2">{channels.length ? channels.map((channel) => <div key={channel.id || channel.displayName} className="rounded-xl border border-[var(--cliente-border)] bg-white p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-bold text-[var(--cliente-card-text)]">{channel.displayName || channel.phoneNumber || "Canal"}</p><StateBadge label={channel.connectionStatus === "connected" || channel.connectionStatus === "ready" ? "Conectado" : "Revisar"} tone={channel.connectionStatus === "connected" || channel.connectionStatus === "ready" ? "success" : "warning"} /></div><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">{channel.phoneNumber || "Número não informado"} · {channel.channelScope === "personal" ? channel.ownerUserName || "Uso pessoal" : "Canal da empresa"}</p></div>) : <p className="rounded-xl border border-dashed border-[var(--cliente-border)] p-3 text-sm text-[var(--cliente-card-text-soft)]">Nenhum canal ligado a este time.</p>}</div></PanelCard><PanelCard className="p-5"><div className="flex items-center justify-between"><div><p className="font-bold text-[var(--cliente-card-text)]">Ranking do time</p><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">Atividade e conversão dos vendedores.</p></div><Link href="/cliente/painel/relatorios?section=sellers" className="text-xs font-bold text-indigo-700">Ranking completo</Link></div><div className="mt-4 space-y-2">{performance.length ? performance.slice().sort((a, b) => Number(b.wonLeads || 0) - Number(a.wonLeads || 0)).slice(0, 5).map((item, index) => <div key={item.ownerId || item.ownerName} className="flex items-center justify-between rounded-xl border border-[var(--cliente-border)] bg-white p-3"><div><p className="text-sm font-bold text-[var(--cliente-card-text)]">{index + 1}. {item.ownerName || "Vendedor"}</p><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">{item.handledChats || 0} atendimentos · {item.avgFirstResponseMinutes || 0} min de resposta</p></div><span className="text-xs font-black text-emerald-700">{item.wonLeads || 0} vendas</span></div>) : <p className="rounded-xl border border-dashed border-[var(--cliente-border)] p-3 text-sm text-[var(--cliente-card-text-soft)]">O ranking aparecerá quando houver atividade atribuída.</p>}</div></PanelCard></section>; }

function PeopleDirectory({ members, teams, search, setSearch, canManage, onChange, onSave }: { members: Member[]; teams: Team[]; search: string; setSearch: (value: string) => void; canManage: boolean; onChange: (index: number, patch: Partial<Member>) => void; onSave: (member: Member) => void }) {
  const visible = members.filter((member) => !search.trim() || `${member.name || ""} ${member.email || ""}`.toLowerCase().includes(search.trim().toLowerCase()));
  return <PanelCard className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold text-[var(--cliente-card-text)]">Pessoas da empresa</p><p className="mt-1 text-xs text-[var(--cliente-card-text-soft)]">A lista completa fica aqui. Escolha o time, edite o perfil e salve sem abrir outra página.</p></div><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar pessoa" className="w-full rounded-xl border border-[var(--cliente-border)] bg-white px-3 py-2.5 text-sm outline-none sm:w-64" /></div><div className="mt-4 overflow-x-auto"><div className="min-w-[760px] space-y-2">{visible.map((member) => { const index = members.findIndex((item) => item.userId === member.userId); const locked = !canManage || member.role === "client_owner"; const profile = inferClientAccessProfile(member); return <div key={member.userId || member.email} className="grid grid-cols-[1.4fr_1fr_1fr_120px_100px] items-center gap-3 rounded-2xl border border-[var(--cliente-border)] bg-[var(--cliente-surface-muted)] p-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-[var(--cliente-card-text)]">{member.name || "Sem nome"}</p><p className="truncate text-xs text-[var(--cliente-card-text-soft)]">{member.email || "Sem e-mail"}</p></div><select disabled={locked} value={member.team || ""} onChange={(event) => onChange(index, { team: event.target.value })} className="rounded-xl border border-[var(--cliente-border)] bg-white px-2.5 py-2 text-xs"><option value="">Sem time</option>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select><select disabled={locked} value={profile.id} onChange={(event) => { const next = getClientAccessProfile(event.target.value); onChange(index, { accessProfile: next.id, role: next.role, capabilities: next.capabilities }); }} className="rounded-xl border border-[var(--cliente-border)] bg-white px-2.5 py-2 text-xs"><option value="seller">Vendedor</option><option value="manager">Gestor</option><option value="support">Atendimento</option><option value="analyst">Analista</option></select><span className="text-xs text-[var(--cliente-card-text-soft)]">{member.allowedChannels?.join(", ") || "Todos os canais"}</span><button type="button" disabled={locked} onClick={() => onSave(member)} className="inline-flex items-center justify-center gap-1 rounded-xl border border-indigo-200 bg-white px-2 py-2 text-xs font-bold text-indigo-700"><Save className="h-3.5 w-3.5" />Salvar</button></div>; })}{!visible.length && <p className="rounded-xl border border-dashed border-[var(--cliente-border)] p-5 text-sm text-[var(--cliente-card-text-soft)]">Nenhuma pessoa encontrada.</p>}</div></div></PanelCard>;
}
...
function PersonComposer({ draft, setDraft, teams, saving, onClose, onSubmit }: { draft: DraftPerson & { team: string }; setDraft: React.Dispatch<React.SetStateAction<DraftPerson & { team: string }>>; teams: Team[]; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 p-0 sm:items-center sm:p-6"><div role="dialog" aria-modal="true" className="w-full max-w-xl rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-indigo-600">Nova pessoa</p><h2 className="mt-1 text-2xl font-bold text-slate-950">Adicionar pessoa à operação</h2><p className="mt-1 text-sm text-slate-600">Convide alguém e já defina o time e o nível de acesso.</p></div><button type="button" onClick={onClose} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><form onSubmit={onSubmit} className="mt-6 space-y-4"><div className="grid gap-3 sm:grid-cols-2"><Field label="Nome" value={draft.name} onChange={(value) => setDraft((current) => ({ ...current, name: value }))} /><Field label="E-mail de acesso" value={draft.email} onChange={(value) => setDraft((current) => ({ ...current, email: value }))} /></div><label className="block space-y-1.5"><span className="text-xs font-bold text-slate-600">Time</span><select value={draft.team} onChange={(event) => setDraft((current) => ({ ...current, team: event.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="">Sem time por enquanto</option>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label><div className="grid gap-3 sm:grid-cols-2"><label className="block space-y-1.5"><span className="text-xs font-bold text-slate-600">Perfil</span><select value={draft.accessProfile} onChange={(event) => setDraft((current) => ({ ...current, accessProfile: event.target.value as ClientAccessProfileId }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="seller">Vendedor</option><option value="manager">Gestor</option><option value="support">Atendimento</option><option value="analyst">Analista</option></select></label><label className="block space-y-1.5"><span className="text-xs font-bold text-slate-600">Capacidade de conversas</span><input type="number" min={1} max={200} value={draft.maxOpenChats} onChange={(event) => setDraft((current) => ({ ...current, maxOpenChats: event.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm" /></label></div><div className="flex justify-end gap-2 border-t border-slate-200 pt-4"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100">Cancelar</button><button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[var(--cliente-primary)] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />}{saving ? "Adicionando" : "Adicionar pessoa"}</button></div></form></div></div>;
}
