...
export function useAdaptivePolling({
  ...
  useEffect(() => {
    ...
    const computeDelay = () => {
      ...
      if (networkInfo?.saveData || effectiveType.includes("2g")) delay = Math.max(delay, slowIntervalMs);
      return delay;
    };
    ...
    const run = async (forced = false) => {
      ...
      inFlight = true;
      emitClienteRealtimePollEvent({ phase: "start", at: Date.now(), source: sourceLabel });

      try {
        await onTickRef.current();
        emitClienteRealtimePollEvent({ phase: "success", at: Date.now(), source: sourceLabel });
      } catch (error) {
        emitClienteRealtimePollEvent({
          phase: "error",
          at: Date.now(),
          source: sourceLabel,
          error: error instanceof Error ? error.message : "Falha ao atualizar em segundo plano.",
        });
      } finally {
        inFlight = false;
      }
    ...
    const onEnvironmentChange = () => schedule();
