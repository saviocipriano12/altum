  ...
  return (
    <SiteShell>
      <section className="relative overflow-hidden px-5 pb-18 pt-24 text-center lg:px-8 lg:pb-24 lg:pt-32">
        <div className="absolute left-1/2 top-0 h-[28rem] w-[52rem] -translate-x-1/2 rounded-full bg-[#2563eb]/14 blur-[140px]" />
        <div className="relative mx-auto max-w-4xl">
          <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#60a5fa]">Planos Altum</p>
      ...
      <section className="border-y border-white/8 bg-[#f3f1ec] px-5 py-24 text-black lg:px-8 lg:py-28">
        <div className="mx-auto grid max-w-[1180px] gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
          <div><Sparkles className="h-7 w-7 text-[#2563eb]" /><h2 className="mt-6 max-w-[11ch] text-[clamp(2.7rem,5vw,5rem)] font-extrabold leading-[0.94] tracking-[-0.06em]">A base comercial já começa conectada.</h2><p className="mt-6 text-lg leading-8 text-black/54">Todos os caminhos partem do mesmo princípio: menos informação espalhada e mais continuidade entre atendimento, venda e gestão.</p></div>
          <div className="grid gap-3 sm:grid-cols-2">{included.map((item) => <div key={item} className="flex min-h-24 items-start gap-3 rounded-2xl border border-black/10 bg-white p-5 font-bold leading-6"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#2563eb]" />{item}</div>)}<div className="flex min-h-24 items-start gap-3 rounded-2xl bg-black p-5 font-bold leading-6 text-white"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-[#60a5fa]" />IA e automações evoluem conforme o plano</div></div>
        </div>
      </section>

      <section className="px-5 py-24 lg:px-8 lg:py-28">
        <div className="mx-auto max-w-4xl">
          <div className="text-center"><HelpCircle className="mx-auto h-7 w-7 text-[#60a5fa]" /><h2 className="mt-6 text-[clamp(2.5rem,5vw,4.5rem)] font-extrabold leading-[0.96] tracking-[-0.06em] text-white">Antes de decidir, tire as dúvidas certas.</h2></div>
          ...
          <div className="mt-12 flex flex-col items-center justify-center gap-3 sm:flex-row"><Link href="/contato?interest=demonstracao" className="inline-flex items-center gap-2 rounded-xl bg-[#2563eb] px-7 py-4 text-sm font-extrabold text-white">Conversar sobre meu cenário <ArrowRight className="h-4 w-4" /></Link><Link href="/implantacao" className="rounded-xl border border-white/12 px-7 py-4 text-sm font-extrabold text-white">Conhecer a implantação</Link></div>
        </div>
      </section>
    </SiteShell>
  );
