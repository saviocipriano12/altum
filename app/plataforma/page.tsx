  ...
  return (
    <SiteShell>
      <section className="relative overflow-hidden px-5 pb-20 pt-24 lg:px-8 lg:pb-28 lg:pt-32">
        <div className="absolute left-1/2 top-0 h-[34rem] w-[58rem] -translate-x-1/2 rounded-full bg-[#2563eb]/14 blur-[150px]" />
        <div className="relative mx-auto max-w-[1280px] text-center">
          <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#60a5fa]">Altum Plataforma</p>
          <h1 className="mx-auto mt-6 max-w-[15ch] text-[clamp(3.3rem,7vw,7rem)] font-extrabold leading-[0.9] tracking-[-0.07em] text-white">
            ...
            <span className="block bg-gradient-to-r from-[#93c5fd] via-[#3b82f6] to-[#8b5cf6] bg-clip-text text-transparent">entender, agir e evoluir.</span>
          </h1>
          ...
          <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/contato?interest=demonstracao" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#2563eb] px-7 py-4 text-sm font-extrabold text-white transition hover:bg-[#1d4ed8]">Agendar demonstração <ArrowRight className="h-4 w-4" /></Link>
          ...
          <div className="relative mx-auto mt-16 max-w-[1120px]">
            <div className="absolute -inset-8 rounded-full bg-[#2563eb]/13 blur-[100px]" />
      ...
      <section className="border-y border-white/8 bg-[#080808] px-5 py-7 lg:px-8">
        <div className="mx-auto flex max-w-[1280px] flex-wrap items-center justify-center gap-x-10 gap-y-4 text-sm font-bold text-white/42">
          {["Conversas", "CRM", "Agenda", "Campanhas", "Commerce", "Relatórios", "IA"].map((item) => <span key={item} className="flex items-center gap-2"><Check className="h-4 w-4 text-[#2563eb]" />{item}</span>)}
        </div>
      </section>
          ...
          {groups.map((group, index) => (
            <article key={group.eyebrow} className="grid gap-12 xl:grid-cols-[0.8fr_1.2fr] xl:items-center">
              <div className={index % 2 ? "xl:order-2" : ""}>
                <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#60a5fa]">{group.eyebrow}</p>
                ...
                <div className="mt-8 grid gap-3 sm:grid-cols-2">
                  {group.items.map(([Icon, title, text]) => <div key={title} className="rounded-2xl border border-white/8 bg-[#0b0b0b] p-5"><Icon className="h-5 w-5 text-[#60a5fa]" /><h3 className="mt-4 font-bold text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-white/42">{text}</p></div>)}
                </div>
              </div>
              <div className={`relative ${index % 2 ? "xl:order-1" : ""}`}><div className="absolute -inset-8 rounded-full bg-[#2563eb]/10 blur-[90px]" /><div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#0b0b0b] p-3 shadow-[0_35px_90px_rgba(0,0,0,0.62)]"><Image src={group.image} alt={group.alt} width={1586} height={992} sizes="(min-width: 1280px) 60vw, 96vw" className="h-auto w-full rounded-[1.35rem]" /></div></div>
            </article>
          ))}
        </div>
      </section>
