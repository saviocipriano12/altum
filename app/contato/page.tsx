    ...
    <SiteShell>
      <section className="relative overflow-hidden px-5 pb-14 pt-24 lg:px-8 lg:pb-20 lg:pt-32">
        ...
        <div className="relative mx-auto max-w-[1180px] text-center">
          <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#60a5fa]">Fale com a Altum</p>
          ...
          <div className="mx-auto mt-9 flex max-w-3xl flex-wrap justify-center gap-x-7 gap-y-3 text-sm font-bold text-white/46">{["Conversa com contexto", "Demonstração aplicada ao seu cenário", "Próximo passo sem pressão"].map((item) => <span key={item} className="flex items-center gap-2"><Check className="h-4 w-4 text-[#2563eb]" />{item}</span>)}</div>
        </div>
      </section>
...
function MiniValue({ icon: Icon, title, text }: { icon: typeof Target; title: string; text: string }) {
  return <article className="rounded-2xl border border-white/8 bg-[#0b0b0b] p-5 text-left"><Icon className="h-5 w-5 text-[#60a5fa]" /><h2 className="mt-4 font-bold text-white">{title}</h2><p className="mt-2 text-sm leading-6 text-white/42">{text}</p></article>;
}
