  ...
  return (
    ...
    <div className="text-white">
      <section className="relative overflow-hidden px-6 pb-16 pt-20">
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_18%_18%,rgba(37,99,235,0.22),transparent_40%),radial-gradient(circle_at_85%_0%,rgba(255,255,255,0.08),transparent_30%)]" />
        <div className="mx-auto max-w-6xl">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#60A5FA]">{eyebrow}</p>
            ...
            {benefits.map((benefit, index) => {
              ...
              return (
                <article key={benefit} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  <Icon className="mb-3 text-[#60A5FA]" size={22} />
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {steps.map((step, index) => (
              <article key={step} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <p className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-[#60A5FA]">Passo {index + 1}</p>
          ...
          <div className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            {delivery7Days.map((item, index) => (
              <div key={item} className="flex items-start gap-3">
                <span className="mt-1 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#60A5FA] text-xs font-bold text-white">
                  {index + 1}
                </span>
      ...
      <section className="px-6 pb-24 pt-8">
        <div className="mx-auto max-w-6xl rounded-3xl border border-[#60A5FA]/35 bg-[#60A5FA]/10 p-8 md:p-10">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href={ctaHref} className="inline-flex items-center gap-2 rounded-full bg-[#60A5FA] px-6 py-3 text-sm font-semibold text-white hover:bg-[#3B82F6]">
      ...
      <div className="pointer-events-none fixed bottom-6 right-6 z-40 hidden lg:block">
        <Link
          href={ctaHref}
          className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-[#60A5FA]/60 bg-[#60A5FA] px-6 py-3 text-sm font-semibold text-white shadow-[0_10px_35px_-10px_rgba(37,99,235,0.7)] hover:bg-[#3B82F6]"
        >
          <Rocket size={16} />
          Agendar diagnóstico
        </Link>
