...
export function HomeSystem() {
  return (
    <section id="como-funciona" className="relative border-b border-white/10 bg-[#070707] px-5 py-24 lg:px-8">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_14%,rgba(37,99,235,0.12),transparent_24%)]" />
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {homeSystemPillars.map((item, index) => (
            <AnimatedSection key={item.title} delay={index * 0.05}>
              <article className="group rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.02))] p-6 transition duration-300 hover:-translate-y-1 hover:border-[#2563eb]/25 hover:bg-white/[0.055]">
                <div className="flex items-center justify-between gap-4">
                  <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-[#2563eb]/20 bg-[#2563eb]/10 text-[#93c5fd]">
                    {(() => {
                      const Icon = pillarIcons[index];
                      return <Icon className="h-5 w-5" />;
                    })()}
                  </span>
                  <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/28 transition group-hover:text-[#93c5fd]">
        ...
        <AnimatedSection delay={0.22}>
          <div className="mt-8 rounded-[32px] border border-[#2563eb]/20 bg-[radial-gradient(circle_at_left,rgba(37,99,235,0.18),transparent_24%),rgba(37,99,235,0.08)] px-6 py-5">
