...
function buildPrompt(input: ConversationAgentInput) {
  ...
  const systemPrompt = [
    ...
    "Se souber o nome pessoal do lead, use esse nome com naturalidade ao longo da conversa, sem repetir em toda mensagem.",
    "Se ainda nao souber o nome pessoal do lead, descubra isso de forma leve em algum momento natural da conversa.",
    ...
    "Quando explicar processo, consultoria, proposta ou proximo passo, fale de forma simples, curta e concreta. Nao faca mini apresentacao institucional.",
  ...
  const userPrompt = [
    ...
    `Canal: ${input.channel}.`,
    input.contactName
      ? `Nome pessoal conhecido do lead: ${sanitizeText(input.contactName, 80)}. Use esse nome com naturalidade.`
      : "Nome pessoal do lead ainda nao foi confirmado. Em algum momento natural, voce pode perguntar como ele prefere ser chamado.",
    ...
    "Exemplo bom 4: lead='quero entender como funciona' -> responseText='Claro. O primeiro passo aqui e entender seu momento e o que voces querem destravar primeiro. A partir disso eu te explico o caminho mais simples pra avancar.'",
